...
export const encryptVaultData = async <T>(
    ...
    const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: ivBytes }, key, plainBytes)
...
export const decryptVaultData = async <T>(storedVault: StoredVault, key: CryptoKey): Promise<T> => {
